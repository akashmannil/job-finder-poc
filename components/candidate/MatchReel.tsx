...
export function MatchReel({ results }: { results: MatchResult[] }) {
  ...
  const caughtUpTitle = useVariant(R.caughtUpTitle);
  const caughtUpBody = useVariant(R.caughtUpBody);
  const emptyMsg = useVariant(R.empty);
  ...
  return (
    <>
      <SwipeDeck
        items={results}
        getKey={(r) => r.jobId}
        right={{ label: R.apply, color: "var(--success)", glyph: "♥" }}
        left={{ label: R.skip, color: "var(--danger)", glyph: "✕" }}
        onSwipe={(r, dir) => {
        ...
        empty={<p className="text-sm text-muted">{emptyMsg}</p>}
        done={
          <div className="card grid place-items-center p-10 text-center">
            <p className="text-lg font-semibold">{caughtUpTitle}</p>
            <p className="mt-1 text-sm text-muted">{caughtUpBody}</p>
          </div>
        }
      />
  ...
  return (
    <article className="card flex h-full flex-col gap-4 overflow-hidden p-6">
      <header className="flex items-start gap-4">
        ...
        {applied && <span className="chip !text-success">{M.applied} ✓</span>}
      </header>
